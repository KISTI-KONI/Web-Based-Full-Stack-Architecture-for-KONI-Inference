        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if(pd['type']=='user'){
                    ...
                    pagenation($('.pagebox'),res['totcount'],numofList,numofPage,page);
                }else if(pd['type']=='good' || pd['type']=='bad' || pd['type']=='comment'|| pd['type']=='rag'|| pd['type']=='normal'){
                    setPage(res);
                    pagenation($('.pagebox'),res['totcount'],numofList,numofPage,page);
                }else if(pd['type']=='normal'){
                    numofList = res['inscount'];
                    setPage(res);
		    $('.pagebox').html('');
                }
...
function setPage(page){
    let retrieval = '&retrieval='+query.get('retrieval')?'&retrieval='+query.get('retrieval'):'';
...
function setPage(res){
    ...
    for(let d of dialogue){
        ...
        instructionHtml += `
        <div class="dialogue flex `+bias+` ursor-pointer" ff='goPage("`+pid+`")'>
            <div class="iid">
                <div>`+icount+`</div>
                <div>`+biasIcon+`</div>
...
function pagenation(pageTarget,total,numofList,numofPage,now){
    ...
    let pageHtml = ''
    if(now != 1){
        pageHtml+=`<div class="pagenum prev cursor-pointer" onclick='setPage(1)'><i class="fa-solid fa-angles-left"></i></div>`
    }
    if(block != 0){
        let tmpnum = numofPage*(block-1)+5
        pageHtml+=`<div class="pagenum prev cursor-pointer" onclick='setPage(`+tmpnum+`)'><i class="fa-solid fa-angle-left"></i></div>`
    }
    for(let i = start;i<=end;i++){
        if(i==now)
            pageHtml+=`<div class="pagenum cursor-pointer now" onclick='setPage(`+i+`)'>`+i+`</div>`
        else
            pageHtml+=`<div class="pagenum cursor-pointer" onclick='setPage(`+i+`)'>`+i+`</div>`
    }    
    if(block != Math.floor(1-(maxPage/numofPage))){
        let tmpnum = numofPage*(block+1)+1

        pageHtml+=`<div class="pagenum prev cursor-pointer" onclick='setPage(`+tmpnum+`)'><i class="fa-solid fa-angle-right"></i></div>`
    }
    if(now != maxPage){
        pageHtml+=`<div class="pagenum prev cursor-pointer" onclick='setPage(`+maxPage+`)'><i class="fa-solid fa-angles-right"></i></div>`
    }
